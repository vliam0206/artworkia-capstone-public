...
public class FirebaseService : IFirebaseService
{
    ...
    public async Task<string?> UploadFileToFirebaseStorage(IFormFile files, string imageName, string folderName)
    {
        ...
        if (files.Length > 0)
        {
            ...
            using (FileStream fileStream = new FileStream(_environment.WebRootPath + $"\\{folderName}\\" + imageName, FileMode.OpenOrCreate))
            {
            ...
            File.Delete(_environment.WebRootPath + $"\\{folderName}\\" + imageName);
        }
